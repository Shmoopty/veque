...
#ifndef VEQUE_EXCEPTIONS
#  if defined(__cpp_exceptions) || defined(_EXCEPTIONS) || defined(_HAS_EXCEPTIONS)
#    define VEQUE_EXCEPTIONS 1
#  else
#    define VEQUE_EXCEPTIONS 0
# endif
#endif
...
namespace veque
{
    ...
    class veque
    {
        ...
        template<typename OtherResizeTraits>
        veque(veque<T, OtherResizeTraits, Allocator> &&other, const Allocator &alloc) noexcept : veque(alloc)
        {
        ...
        reference at(size_type idx)
        {
            ...
            {
#if VEQUE_EXCEPTIONS
                throw std::out_of_range("veque::at(" + std::to_string(idx) + ") out of range");
#else
                assert(!"veque::at(idx): out of range");
#endif
            }
            return (*this)[idx];
        }
        ...
        void reserve(size_type front, size_type back)
        {
            if (front > capacity_front() || back > capacity_back())
            {
                ...
                {
#if VEQUE_EXCEPTIONS
                    throw std::length_error("veque::reserve(" + std::to_string(front) + ", " + std::to_string(back) + ") exceeds max_size()");
#else
                    assert(!"veque::reserve(front, back): length error");
#endif
                }
    ...
    private:
        ...
        veque(InputIt b, InputIt e, const Allocator &alloc, std::input_iterator_tag) : veque { alloc }
        {
            for (; b != e; ++b)
                push_back(*b);
        }
