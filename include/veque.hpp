...
#include <utility>
    ...
    private:
        ...
        decltype(auto) _nothrow_move( T & t );
    ...
    void veque<T,Alloc>::_destroy( const_iterator b, const_iterator e )
    {
        if constexpr ( ! std::is_trivially_destructible_v<T> )
        {
            ...
            {
                std::allocator_traits<Alloc>::destroy( _allocator(), i );
            }
        }
    }
    ...
    void veque<T,Alloc>::_construct_value( const_iterator b, const_iterator e, const Args & ...args )
    {
        ...
        {
            std::allocator_traits<Alloc>::construct( _allocator(), dest, args... );
        }
    }
    ...
    {
        for ( auto dest = begin() + std::distance(cbegin(),b); dest != e; ++dest, ++src )
        {
    ...
    veque<T,Alloc>::veque( const veque &other )
        : _size{ other._size }
        , _offset{ 0 }
        , _data { other._size, std::allocator_traits<Alloc>::select_on_container_copy_construction( other._allocator() ) }
    {
        ...
        , _data{ alloc }
    {
        ...
        else
        {
            ...
            else
            {
                ...
                auto replacement = veque( allocate_uninitialized_tag{}, other.size(), _allocator() );
                if constexpr ( std::is_trivially_copyable_v<T> )
                {
    ...
    veque<T,Alloc> & veque<T,Alloc>::operator=( const veque & other )
    {
        ...
        else
        {
            ...
            {
                _swap_with_allocator( veque( other, _allocator() ) );
                return *this;
            }
        }

        _destroy( begin(), end() );
        auto new_begin = _data._storage + (capacity_full() - other.size()) / 2;
        auto new_end = new_begin + other.size();
        _copy_construct_range( new_begin, new_end, other.begin() );
        _move_begin( new_begin - begin() );
        _move_end( new_end - end() );
        return *this;
    }
    ...
    typename veque<T,Alloc>::size_type veque<T,Alloc>::max_size() const noexcept
    {
        ...
        auto runtime_limit = std::allocator_traits<Alloc>::max_size(_allocator());
    ...
    void veque<T,Alloc>::_resize_back( size_type count, const Args & ... args )
    {
        ...
        if ( count > size() )
        {
            ...
            _construct_value( end(), begin() + count, args... );
        }
        else
        {
    ...
    template< typename T, typename Alloc >
    template< typename ...Args >
    void veque<T,Alloc>::_resize_front( veque<T,Alloc>::size_type count, const Args & ...args )
    {
        difference_type delta = count - size();
        if ( delta > 0 )
        {
            ...
            _construct_value( new_begin, begin(), args... );
        }
        else
        {
            auto new_begin = begin() - delta;
            _destroy( begin(), new_begin );
        }
        _move_begin( -delta );
    }
    ...
    void veque<T,Alloc>::reserve_front( veque<T,Alloc>::size_type count )
    {
        if ( count > capacity_front() )
        {
            _reallocate( capacity_back() + count, count );
        }
    }
    ...
    void veque<T,Alloc>::reserve_back( veque<T,Alloc>::size_type count )
    {
        if ( count > capacity_back() )
        {
            _reallocate( capacity_front() + count, _offset );
        }
    }
    ...
    typename veque<T,Alloc>::reference veque<T,Alloc>::emplace_back(Args && ... args)
    {
        ...
        std::allocator_traits<Alloc>::construct( _allocator(), end(), std::forward<Args>(args)... );
        _move_end( 1 );
        return back();
    }
    ...
    typename veque<T,Alloc>::reference veque<T,Alloc>::emplace_front(Args && ... args)
    {
        ...
        std::allocator_traits<Alloc>::construct( _allocator(), begin()-1, std::forward<Args>(args)... );
        _move_begin( -1 );
        return front();
    }
    ...
    typename veque<T,Alloc>::iterator veque<T,Alloc>::emplace( typename veque<T,Alloc>::const_iterator it, Args && ... args )
    {
        ...
        std::allocator_traits<Alloc>::construct( _allocator(), res, std::forward<Args>(args)... );
        return res;
    }
    ...
    {
        auto res = _insert_storage( it, 1 );
        std::allocator_traits<Alloc>::construct( _allocator(), res, val );
        return res;
    }
    ...
    {
        auto res = _insert_storage( it, 1 );
        std::allocator_traits<Alloc>::construct( _allocator(), res, std::move(val) );
        return res;
    }
    ...
    typename veque<T,Alloc>::iterator veque<T,Alloc>::insert( typename veque<T,Alloc>::const_iterator it, InputIt first, InputIt last )
    {
        ...
        auto res = _insert_storage( it, cnt );
        _copy_construct_range( res, res + cnt, first );
        return res;
    }
